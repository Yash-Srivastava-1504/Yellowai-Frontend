...
import { Bot, Settings, Menu, X, ChevronLeft, ChevronRight } from "lucide-react";
import { useAuth } from "@/contexts/AuthContext";

const navItems = [
  { icon: Bot, label: "My Agents", path: "/projects" },
...
export default function DashboardLayout() {
  ...
  const displayName = profile?.anonymous ? "Anonymous" : (profile?.display_name?.trim() || "User");
          ...
          <aside className="absolute left-0 inset-y-0 w-56 bg-background border-r border-border p-5 animate-fade-in">
            <div className="flex justify-between items-center mb-6">
              <span className="flex items-center gap-2 text-lg font-bold text-foreground">
                <Bot className="w-6 h-6 text-primary" /> AgentForge
              </span>
